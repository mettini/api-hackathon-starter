...
class AppComponents(context: Context) extends BuiltInComponentsFromContext(context)
  ...
  // lazy val csrfFilter = new CSRFFilter(CSRFConfig.fromConfiguration(configuration), csrfTokenSigner, new SignedTokenProvider(crypto))
  override lazy val httpFilters: Seq[EssentialFilter] = Seq(gzipFilter) // add csrfFilter to enabled
