...
class EmailVerificationRepo(val db: Database) extends BaseModel[EmailVerification] {
  ...
  def updateVerificationStatusVerified(id: Long): Either[String, Long] =
    update(id, Seq(NamedParameter("verification_status", VERIFIED.value)))

  def updateDeleteEmailVerification(id: Long): Either[String, Long] =
    update(id, Seq(NamedParameter("is_deleted", true)))

  private def update(id: Long, o: Seq[NamedParameter]): Either[String, Long] = {
     db.withConnection { implicit connection =>
      val updateSQL = " update email_verifications set " + o.map(k => s"${k.name}={${k.name}}").mkString(",") +
        ", last_modification_date = NOW() where id = {id} "
      SQL(updateSQL).on(o :+ NamedParameter("id", id): _*).executeUpdate() match {
        case 1 => Right(1)
        case 0 => Left(s"there was an error updating the email_verifications #$id for parameters ${o.toString}")
      }
    }
  }
