...
case class UserRepo(userConfigRepo: UserConfigRepo, db: Database) extends BaseModel[User] {
  ...
  def habeasData(userId: Long): Either[String, Long] = {
  ...
  def updateVerificationStatusActive(userId: Long): Either[String, Long] =
    update(userId, Seq(NamedParameter("verification_status", UserVerificationStatus.Active.toString)))

  def moderateUser(userId: Long): Either[String, Long] =
  ...
  private def habeasDataEmail(user: User): Either[String, Long] = {
  ...
  def update(userId: Long, o: Seq[NamedParameter]): Either[String, Long] = {
     db.withConnection { implicit connection =>
      val updateSQL = " update users set " + o.map(k => s"${k.name}={${k.name}}").mkString(",") + ", last_modification_date = NOW() where id = {id} "
      SQL(updateSQL).on(o :+ NamedParameter("id", userId): _*).executeUpdate() match {
        case 1 => Right(1)
        case 0 => Left(s"there was an error updating the user #$userId for parameters ${o.toString}")
      }
    }
  }
}
