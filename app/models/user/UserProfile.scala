...
class UserProfileRepo(val db: Database) extends BaseModel[UserProfile] {
  ...
  private def update(userId: Long, o: Seq[NamedParameter]): Either[String, Int] = {
    db.withConnection { implicit connection =>
      val updateSQL = " update user_profiles set " + o.map(k => s"${k.name}={${k.name}}").mkString(",") +
        ", last_modification_date = NOW() where user_id = {userId}"
      SQL(updateSQL).on(o :+ NamedParameter("userId", userId): _*).executeUpdate() match {
        case 1 => Right(1)
        case 0 => Left(s"there was an error updating user profile for user #$userId")
      }
    }
  }
}
